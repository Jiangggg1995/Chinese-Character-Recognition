...
def build_graph(top_k):
    ...
    with tf.device('/cpu:0'):
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            # images shape([None, 64, 64, 1])
            conv3_1 = slim.conv2d(images, 64, [3, 3], 1, padding='SAME', scope='conv3_1')
            max_pool_1 = slim.max_pool2d(conv3_1, [2, 2], [2, 2], padding='SAME', scope='pool1')

            # max_pool_1 shape([None, 32, 32, 64])
            conv3_2 = slim.conv2d(max_pool_1, 128, [3, 3], padding='SAME', scope='conv3_2')
            max_pool_2 = slim.max_pool2d(conv3_2, [2, 2], [2, 2], padding='SAME', scope='pool2')

            # max_pool_2 shape([None, 16, 16, 128])
            conv3_3 = slim.conv2d(max_pool_2, 256, [3, 3], padding='SAME', scope='conv3_3')
            max_pool_3 = slim.max_pool2d(conv3_3, [2, 2], [2, 2], padding='SAME', scope='pool3')

            # max_pool_3 shape([None, 8, 8, 256])
            conv3_4 = slim.conv2d(max_pool_3, 512, [3, 3], padding='SAME', scope='conv3_4')
            conv3_5 = slim.conv2d(conv3_4, 512, [3, 3], padding='SAME', scope='conv3_5')
            max_pool_4 = slim.max_pool2d(conv3_5, [2, 2], [2, 2], padding='SAME', scope='pool4')

            # max_pool_4 shape([None, 4, 4, 512])
            flatten = slim.flatten(max_pool_4)
            # flatten shape([None, 4*4*512=8192])
...
def main(_):
    ...
    if FLAGS.mode == "train":
        ...
        image_path = 'C:\\Users\\Jiang\\Desktop\\chineserec\\testresult\\2015002.jpg'
        final_predict_val, final_predict_index = inference(image_path)
        logger.info('the result info label {0} predict index {1} predict_val {2}'.format('test', final_predict_index,
                                                                                         final_predict_val))
